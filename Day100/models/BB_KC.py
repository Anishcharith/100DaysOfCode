...
def buy(open_p,close_p,low_p,high_p,volume):
    buy=[0]
    # tech indicators
    middle_line,upper_line,lower_line = KeltnerChannel(close_p,high_p,low_p)
    middle_band,upper_band,lower_band = BB(close_p)
    for i in range(1,len(close_p)):
        # buy condition
        if upper_band[i]>upper_line[i] and close_p[i]>=upper_line[i]:
            buy.append(1)
        else:
            buy.append(0)
    return buy
