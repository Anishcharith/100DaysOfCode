...
sys.path.insert(0,'../Day013/')
sys.path.insert(0,'../Day022/')
from Alligator import Alligator
from ChoppinessIndex import ChoppinessIndex
# --------------------

def buy(open_p,close_p,low_p,high_p,volume):
    buy=[0]
    jaw,teeth,lip=Alligator(close_p)
    ci=ChoppinessIndex(close_p,high_p,low_p)
    for i in range(1,len(close_p)):
        if ci[i]<50 and lip[i]>teeth[i] and teeth[i]>jaw[i]:
            buy.append(1)
        else:
            buy.append(0)
    return buy
