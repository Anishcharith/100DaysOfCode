...
def main(comp,days,start=0):
    ...
    while(i<len(buy_a)):
        ...
        if buystate and (sell_a[i]==1 or stoploss_a[i]==1):
            ...
            if sell_res[-1]>buy_res[-1] and alpha==1:
                beta+=1
            elif sell_res[-1]>buy_res[-1] and alpha==-1:
                beta/=2
                if beta>-1:
                    beta=1
                    alpha=1
    ...
    total=100*np.sum([k*(i-j)/(j+1) for (i,j,k) in zip(sell_res,buy_res,longshort)])
    f.write(str(total))
